...
#SBATCH --constraint=v100-32gb           # Target A100 GPUs specifically a100|
...
#SBATCH --output=%u.evaluate_sft2   # Log file location with meaningful name
...
#SBATCH --job-name=evaluate_model_sft   # More descriptive job name
...
# Set training type - change this to evaluate different models
# Options: "finetuning" or "consistency"
training_type="finetuning"

# Path to LoRA weights based on training type
lora_weights="lora_weight/LLaMA2-7B-Chat/${training_type}/original"

# Use empty string to evaluate base model without LoRA
# lora_weights=""

# Common parameters for both runs
common_params="--dataset SQuAD --parallel_datasets  \
    --model $model \
...
if [ -n "$lora_weights" ]; then
    echo "=== Evaluating model with ${training_type} LoRA weights ==="
    echo "=== LoRA weights path: ${lora_weights} ==="
    # Provide only the filename base pattern, directory will be handled by evaluate.py
    python evaluate.py $common_params --lora_weights "$lora_weights" --evaluate_with_gpt --output_file "${model}_${training_type}"
else
    echo "=== Evaluating base model without LoRA ==="
    # Provide only the filename base pattern
    python evaluate.py $common_params --evaluate_with_gpt --output_file "${model}_base"
fi
